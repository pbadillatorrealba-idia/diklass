...
integración viva se ejecutan en el clúster PostgreSQL scratch `/tmp/verify-003/` (shims de roles
de API, `auth.users`, `auth.uid()`, `auth.jwt()` y pgtap; técnica del quickstart de 002) y en el
job `database` de CI: su «rojo» previo se registra con su URL o su salida en `quickstart.md`, que
se crea al comenzar la implementación y se va llenando de forma incremental (consolidación final
en 5.3). Las pruebas de unidad corren localmente con `bun test`. Cada tarea indica su criterio
observable de terminación y su trazabilidad. Requisito de integración ya declarado en `design.md`
(RI-1): `src/lib/supabase/database.types.ts` es archivo compartido prohibido y su regeneración con
`supabase gen types` la ejecuta el orquestador; el paso de diff de tipos del job `database` queda
en rojo declarado hasta entonces.

## 1. Base de datos: garantías de la spec (pruebas primero)

- [ ] 1.1 Escribir `supabase/tests/009_base_conocimiento.sql` (pgTap, patrón de
  ...
  la suite falla por los objetos inexistentes y las aserciones nuevas; «rojo» observado en
  `/tmp/verify-003/` y en el job `database` de CI con su URL registrada en `quickstart.md`.
- [ ] 1.2 Escribir `supabase/migrations/010_base_conocimiento.sql`: tablas `knowledge_documents`
  ...
  Verificación: suite `009` en verde en el clúster scratch y en el job `database` de CI, suites
  `001`–`008` siguen verdes con la 010 aplicada, y el paso de `supabase gen types` queda declarado
  en rojo por RI-1 (registrado en `quickstart.md`).
- [ ] 1.3 Confirmar y documentar en `quickstart.md` que las transiciones de fuente emiten
...
- [ ] 2.1 Escribir en rojo `tests/unit/conocimiento/schema.test.ts`: esquemas Zod de `design.md`
  ...
  `sin_paciente_seleccionado`, `fuente_retirada`, `sin_terminos_consultables`) y forma persistida
  de `knowledge_queries` (FR-020 · US5-AC5). Verificación: `bun test tests/unit` en rojo por los
  esquemas inexistentes; luego verde.
- [ ] 2.2 Escribir en rojo `tests/unit/conocimiento/answer.test.ts` y luego implementar
...
- [ ] 2.3 Escribir en rojo `tests/unit/conocimiento/conversation-store.test.ts` y luego implementar
...
- [ ] 3.1 Escribir en rojo `tests/unit/conocimiento/coleccion-service.test.ts` y
...
- [ ] 3.2 Escribir en rojo `tests/unit/conocimiento/consulta-service.test.ts` y
...
- [ ] 3.3 Escribir en rojo y luego completar `tests/integration/conocimiento/evaluacion.test.ts`
  ...
  verbatim; su revisión manual queda pendiente de aceptación). Verificación: el arnés pasa sobre el
  clúster scratch y en el job `database` de CI con las métricas impresas en su salida; resultados
...
- [ ] 4.1 Implementar la ruta `/knowledge` con componentes de `src/components/conocimiento`:
  ...
  `src/app/(protected)/home.tsx` como edición mínima de 1–2 líneas documentada en `quickstart.md`
  (coordinada por hub con las ramas hermanas; si no es coordinable, RI-2). Etiquetas
  ...
  verdes; verificación visual declarada pendiente.
- [ ] 4.2 Implementar `/knowledge/sources`, `/knowledge/sources/new` y `/knowledge/sources/[id]`:
...
- [ ] 5.1 Crear `quickstart.md` del cambio al comenzar la implementación y mantenerlo incremental:
  guion reproducible del clúster scratch `/tmp/verify-003/` (roles de API, shims de `auth.*`,
  pgtap, migraciones 001–010 y suites 001–009), «rojo» previo de 1.1 y comandos de las tareas
  siguientes. Verificación: documento con guion ejecutable y resultados reales desde la tarea 1.1.
- [ ] 5.2 Ejecutar las compuertas locales acotadas (`bunx biome check --write` sobre los archivos
  ...
  CI en verde en la rama para las suites 001–009 y la integración viva, con el rojo declarado de
  RI-1 (diff de tipos) anotado. Verificación: lista de comandos y resultados + URLs de CI en
  `quickstart.md`.
- [ ] 5.3 Consolidar `quickstart.md` con la evidencia real acumulada desde 1.1 (ciclos rojo→verde
  por tarea, comandos, URLs, métricas del arnés de evaluación y alcance de lo verificado) y los
  pendientes explícitos: RI-1 (regeneración de `database.types.ts`), corpus real y conjunto anotado
...
- [ ] 6.1 Publicar el reporte final en español (en la PR que crea el orquestador): mapa
  ...
  `design.md`) y requisitos de integración (RI-1, y RI-2 si aplica). Verificación: reporte publicado
  y coherente con `quickstart.md`.
