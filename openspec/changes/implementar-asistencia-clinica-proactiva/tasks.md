...
- [x] 1..1 Escribir `supabase/tests/012_asistencia_clinica.sql` (pgTap, patrón de
  ...
  sellado de las filas al cerrar la consulta —el trigger heredado de 002 acota su sello a los
  registros de trabajo de su spec, así que estas entidades las sella el trigger 013 con la misma
  semántica sobre `old` (FR-024 heredado · D8)—, `approve_clinical_record` sigue rechazando
  no-epicrisis (una hipótesis o sugerencia nunca
...
- [x] 1..2 Escribir `supabase/migrations/013_asistencia_clinica.sql`: función y trigger
  ...
  23514, más el sello `CLINICAL_RECORD_SEALED` de estas entidades sobre `old` —el heredado de 002
  acota su alcance a `('anamnesis','diagnosis','epicrisis')`—, sin duplicar los guardas de
  atribución de 001).
...
- [x] 1..3 Confirmar y documentar en `quickstart.md` qué transiciones emiten cada acción de FR-063
...
- [x] 2..1 Escribir en rojo `tests/unit/asistencia/schema.test.ts`: esquemas Zod y tipos de D2 —
  ...
  luego verde.
- [x] 2..2 Escribir en rojo `tests/unit/asistencia/deteccion.test.ts` y luego implementar
  ...
  rojo→verde.
- [x] 2..3 Escribir en rojo `tests/unit/asistencia/soporte-diferencial.test.ts` y luego implementar
...
- [x] 3..1 Escribir en rojo pruebas de `tests/unit/asistencia/asistencia-service.test.ts` y
...
- [x] 3..2 Escribir en rojo pruebas de `tests/unit/asistencia/hipotesis-service.test.ts` y
...
- [x] 3..3 Escribir y ejecutar `tests/integration/asistencia/evaluacion.test.ts` sobre
...
- [x] 4..1 Implementar la ruta `/support/consultations/[id]` con los componentes de
...
- [x] 4..2 Implementar el panel de soporte diferencial en la misma ruta: botón explícito «Solicitar
...
- [x] 5..1 Crear `quickstart.md` del cambio al comenzar la implementación y mantenerlo
...
- [x] 5..3 Consolidar `quickstart.md` con la evidencia real acumulada desde 1.1 (ciclos
