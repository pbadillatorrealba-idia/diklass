    ...
    <SafeAreaView style={{ backgroundColor: "#f8fafc", flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }} keyboardShouldPersistTaps="handled">
        <VStack className="w-full max-w-[720px] gap-6">
          <Heading size="2xl">Registrar paciente</Heading>
          <Text className="text-foreground/70">
            Los campos opcionales pueden quedar sin dato: el sistema los señala sin inventar valores
            ni confundirlos con hallazgos negativos.
          </Text>
          <FichaForm
            errors={fichaErrors}
            isDisabled={isSaving}
          ...
          {tutorMode === "existing" ? (
            <VStack className="w-full gap-3">
              {tutorsQuery.isLoading ? (
                <Text testID="tutors-loading">Cargando tutores…</Text>
              ) : null}
              {(tutorsQuery.data ?? []).length === 0 && tutorsQuery.isSuccess ? (
                <Text testID="tutors-empty">
                  Aún no hay tutores registrados: registra uno nuevo.
                </Text>
              ) : (
                <OptionPicker
                  ...
                  <FormControl isInvalid={Boolean(error)} key={field}>
                    ...
                    <Input>
                      <InputField
                        ...
                        onChangeText={(text) =>
                          setTutorValues((prev) => ({ ...prev, [field]: text }))
                        }
                        testID={testID}
