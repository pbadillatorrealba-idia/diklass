...
**If NOT ignored:** Add the selected project-local directory to `.git/info/exclude`, then verify it is ignored. This is local Git configuration; do not create a commit while preparing the worktree. If the directory is still not ignored, stop before creating the worktree.
...
git worktree add -b "$BRANCH_NAME" "$path" origin/main
cd "$path"
```
...
| Directory not ignored | Add to `.git/info/exclude` and verify, without a commit |
